...
export async function signInBuyer(page: Page): Promise<void> {
  ...
  await page.locator('input[autocomplete="email"], input[autocomplete="username"]').fill(email);
  await page.locator('input[autocomplete="current-password"]').fill(password);
  await expect(page.locator('.tenant-preview .status-badge:not(.status-badge--inactive)')).toBeVisible({ timeout: 10_000 });

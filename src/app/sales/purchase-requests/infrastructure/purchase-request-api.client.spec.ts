...
describe('PurchaseRequestApiClient', () => {
  ...
  it('creates a request with line set and no client account selector', () => {
    const command = { priority: 'NORMAL' as const, requestedDeliveryDate: null, deliveryProfileSnapshot: '', paymentOption: 'CASH_ON_DELIVERY' as const, comment: '', lines: [{ catalogItemId: 'CAT-001', quantity: 2, unit: 'unit', notes: '' }] };
  ...
  it('sends If-Match and idempotency key on submission', () => {
    client.submit({ id: 'PR-001', version: 5, etag: '"5"' } as PurchaseRequest, 'portal-PR-001-5').subscribe();

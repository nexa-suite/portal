...
import { InventoryAvailabilityApiPort } from '../application/ports/inventory-availability-api.port';
import { InventoryAvailability } from '../domain/inventory-availability.model';

@Injectable({ providedIn: 'root' })
export class InventoryAvailabilityApiClient implements InventoryAvailabilityApiPort {
  ...
  list(ids: readonly string[]): Observable<readonly InventoryAvailability[]> {
    const values = [...new Set(ids.filter(Boolean))].slice(0, 100);
    if (!values.length) return of([]);
    let params = new HttpParams();
    values.forEach((id) => params = params.append('catalogItemIds', id));

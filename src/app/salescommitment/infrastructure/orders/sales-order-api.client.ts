...
import {
  ...
  SalesOrderPage,
  validSalesOrderSort,
} from '../../domain/orders/sales-order.models';
...
export class SalesOrderApiClient implements SalesOrderApiPort {
  ...
  summary(id: string, format: 'PDF' | 'CSV'): Observable<SalesOrderDownload> {
    return this.http.get(this.api(`/${encodeURIComponent(id)}/summary`), {
      observe: 'response',
      responseType: 'blob',
      withCredentials: true,
      params: { format },
    }).pipe(map((response) => ({ body: response.body, contentDisposition: response.headers.get('Content-Disposition') })));
  }

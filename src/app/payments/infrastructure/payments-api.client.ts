...
export class PaymentsApiClient {
  ...
  listPaymentsForReceivable(receivableId: string, page = 0, size = 25): Observable<PaymentPage<PaymentHistoryItem>> {
    const params = new HttpParams().set('page', page).set('size', size);
    return this.http.get<PaymentPage<PaymentHistoryItem>>(this.api(`/receivables/${encodeURIComponent(receivableId)}/payments`), { params, withCredentials: true });
  }
}

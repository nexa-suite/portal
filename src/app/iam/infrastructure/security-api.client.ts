...
export class SecurityApiClient {
  ...
  updateProfile(value: { displayName: string; phone: string; preferredLanguage: string; timezone: string }, version: number): Observable<Profile> { return this.http.patch<Profile>(this.url('/api/v1/me/profile'), value, { headers: { 'If-Match': `"${version}"` } }); }

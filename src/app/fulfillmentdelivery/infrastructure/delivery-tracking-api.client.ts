...
function nullableText(value: unknown): string | null { return typeof value === 'string' && value.length > 0 ? value : null; }
function nullableNumber(value: unknown): number | null { return typeof value === 'number' && Number.isFinite(value) ? value : null; }
function delivery(value: unknown): Delivery { const item=raw(value); const assignment=raw(item['assignment']); return { id:text(item['id']), dispatchNumber:text(item['dispatchNumber']), salesOrderId:nullableText(item['salesOrderId']), salesOrderNumber:text(item['salesOrderNumber']), clientAccountId:nullableText(item['clientAccountId']), status:(text(item['status'])||'UNKNOWN') as Delivery['status'], destination:typeof item['destination']==='string'?item['destination']:null, deliveryWindowStart:typeof item['deliveryWindowStart']==='string'?item['deliveryWindowStart']:null, deliveryWindowEnd:typeof item['deliveryWindowEnd']==='string'?item['deliveryWindowEnd']:null, eta:typeof item['eta']==='string'?item['eta']:null, routeName:nullableText(assignment['routeName']), temperatureMin:nullableNumber(item['temperatureMin']), temperatureMax:nullableNumber(item['temperatureMax']), temperatureUnit:nullableText(item['temperatureUnit']), temperatureStatus:nullableText(item['temperatureStatus']), podStatus:typeof item['podStatus']==='string'?item['podStatus']:null, version:Number(item['version'])||0, updatedAt:text(item['updatedAt']), alerts:Array.isArray(item['alerts'])?item['alerts'].map(String):[] }; }

@Injectable({providedIn:'root'})
export class DeliveryTrackingApiClient implements DeliveryTrackingApiPort {

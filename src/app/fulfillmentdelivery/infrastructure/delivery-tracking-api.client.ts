...
function delivery(value: unknown): Delivery { const item=raw(value); return { id:text(item['id']), dispatchNumber:text(item['dispatchNumber']), salesOrderId:nullableText(item['salesOrderId']), salesOrderNumber:text(item['salesOrderNumber']), clientAccountId:null, status:(text(item['status'])||'UNKNOWN') as Delivery['status'], destination:typeof item['destination']==='string'?item['destination']:null, deliveryWindowStart:typeof item['deliveryWindowStart']==='string'?item['deliveryWindowStart']:null, deliveryWindowEnd:typeof item['deliveryWindowEnd']==='string'?item['deliveryWindowEnd']:null, eta:typeof item['eta']==='string'?item['eta']:null, podStatus:typeof item['podStatus']==='string'?item['podStatus']:null, version:Number(item['version'])||0, updatedAt:text(item['updatedAt']), alerts:Array.isArray(item['alerts'])?item['alerts'].map(String):[] }; }
